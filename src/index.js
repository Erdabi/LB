  ...
  return div({ class: "p-4" }, [
    div([
      ...
      input({
        ...
        value: model.answer,
        oninput: (event) =>
          dispatch({ type: MSG.CHANGE_ANSWER, value: event.target.value }),
        class: "mr-10",
      }),
      button(
        {
    ...
    ...model.cards.map((card, index) => div(
        { key: index, class: "border-yellow-600 w-60 break-words relative m-10 p-10"},[
          ...
          button(
            {
              onclick: () => dispatch({ type: MSG.TOGGLE_ANSWER, index }),
            },
...
function update(message, model) {
  switch (message.type) {
    case MSG.CHANGE_QUESTION:
      return { ...model, question: message.value };

    case MSG.CHANGE_ANSWER:
